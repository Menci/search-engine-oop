...
void HTMLDocument::internelGetTextContent(const DOM::NodeWithChildren *node, StringEx &result, bool ignoreScriptAndStyle) {
    for (const std::shared_ptr<DOM::Node> &child : node->children) {
        if (auto childElement = std::dynamic_pointer_cast<DOM::ElementNode>(child)) {
            if (ignoreScriptAndStyle && (childElement->tagName.toUpper() == "SCRIPT" || childElement->tagName.toUpper() == "STYLE")) continue;
            HTMLDocument::internelGetTextContent(childElement.get(), result, ignoreScriptAndStyle);
